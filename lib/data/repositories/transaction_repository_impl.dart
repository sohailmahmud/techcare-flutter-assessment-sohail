...
class TransactionRepositoryImpl implements TransactionRepository {
      ...
      if (isOnline) {
        try {
          ...
          final cacheResponse = PaginatedTransactionsResponse(
            data: remoteResponse.data.map((transaction) => TransactionModel.fromEntity(transaction)).toList(),
          ...
          final cachedResponse = await _localDataSource.getCachedTransactions(query);
    ...
    try {
      ...
      if (isOnline) {
        try {
          ...
          await _localDataSource.cacheTransaction(TransactionModel.fromEntity(transaction));
        ...
        return const Left(NetworkFailure('No internet connection and transaction not cached'));
      }
    } catch (e) {
    ...
    try {
      ...
      if (isOnline) {
        try {
          ...
          final remoteTransaction = await _remoteDataSource.createTransaction(transaction);
          
          // Cache the created transaction
          await _localDataSource.cacheTransaction(TransactionModel.fromEntity(remoteTransaction));
    ...
    try {
      ...
      if (isOnline) {
        try {
          ...
          final remoteTransaction = await _remoteDataSource.updateTransaction(transaction);
  ...
  Future<Either<Failure, List<Transaction>>> getCachedTransactions() async {
    try {
      ...
      if (cachedResponse != null) {
        return Right(cachedResponse.data.map((model) => model.toEntity()).toList());
      }
  ...
  Future<Either<Failure, void>> syncOfflineChanges() async {
    try {
      ...
      for (final operation in pendingOperations) {
        ...
        try {
          switch (operation.operationType) {
            case 'create':
              final transactionModel = TransactionModel.fromJson(operation.data);
              await _remoteDataSource.createTransaction(transactionModel.toEntity());
              break;
            case 'update':
              final transactionModel = TransactionModel.fromJson(operation.data);
              await _remoteDataSource.updateTransaction(transactionModel.toEntity());
              break;
            case 'delete':
          ...
          if (operation.retryCount < 3) {
            ...
            failures.add('${operation.operationType} ${operation.resourceType}: ${operation.toString()}');
            await _localDataSource.removePendingOperation(operation.id);
          }
        }
      }
      ...
      if (failures.isNotEmpty) {
        return Left(SyncFailure('Some operations failed: ${failures.join(', ')}'));
      }
