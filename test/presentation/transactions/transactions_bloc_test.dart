...
void main() {
  ...
  group('TransactionsBloc cancellation', () {
    test('ignores stale load response when a newer load is issued', () async {
      ...
      final slowResponse = PaginatedResponse<tx.Transaction>(
        data: [
          tx.Transaction(
            ...
            category: const cat.Category(id: 'lc1', name: 'LC1', icon: Icons.category, color: Colors.green),
            date: DateTime.now(),
          ),
        ],
      ...
      final fastResponse = PaginatedResponse<tx.Transaction>(
        data: [
          tx.Transaction(
            ...
            category: const cat.Category(id: 'lc2', name: 'LC2', icon: Icons.category, color: Colors.orange),
            date: DateTime.now(),
          ),
        ],
        meta: const PaginationMeta(currentPage: 1, totalPages: 1, totalItems: 1, itemsPerPage: 20, hasMore: false),
      ...
      const emptyResponse = PaginatedResponse<tx.Transaction>(
        data: [],
        meta: PaginationMeta(currentPage: 1, totalPages: 1, totalItems: 0, itemsPerPage: 20, hasMore: false),
      );
  ...
  await sub.cancel();
      ...
      final lastLoaded = states.lastWhere((s) => s is TransactionLoaded) as TransactionLoaded;
    ...
    test('ignores stale filter response when a newer filter is issued', () async {
      final bloc = TransactionsBloc(
        cacheManager: MockHiveCacheManager(),
        transactionRepository: mockRepo,
      );
      ...
      final slowResponse = PaginatedResponse<tx.Transaction>(
        data: [
          tx.Transaction(
            ...
            amount: 7.0,
            type: tx.TransactionType.income,
            category: const cat.Category(id: 'fc1', name: 'FC1', icon: Icons.category, color: Colors.purple),
            date: DateTime.now(),
          ),
        ],
        meta: const PaginationMeta(currentPage: 1, totalPages: 1, totalItems: 1, itemsPerPage: 20, hasMore: false),
      ...
      final fastResponse = PaginatedResponse<tx.Transaction>(
        data: [
          tx.Transaction(
            ...
            amount: 8.0,
            type: tx.TransactionType.expense,
            category: const cat.Category(id: 'fc2', name: 'FC2', icon: Icons.category, color: Colors.brown),
            date: DateTime.now(),
          ),
        ],
        meta: const PaginationMeta(currentPage: 1, totalPages: 1, totalItems: 1, itemsPerPage: 20, hasMore: false),
  ...
  await sub.cancel();
      ...
      final lastLoaded = states.lastWhere((s) => s is TransactionLoaded) as TransactionLoaded;
      expect(lastLoaded.transactions.any((t) => t.id == 'filter_fast'), isTrue);
      expect(lastLoaded.transactions.any((t) => t.id == 'filter_slow'), isFalse);
