  ...
  group('TransactionsBloc optimistic add and id map', () {
    ...
    setUp(() {
      ...
      when(
        ...
        (_) async => const Right(
          PaginatedResponse<tx.Transaction>(
            data: [],
            meta: PaginationMeta(
              currentPage: 1,
              totalPages: 1,
    ...
    test(
      'adds optimistic transaction and replaces temp id with server id without duplicates',
      () async {
        ...
        final finalLoaded =
            states.lastWhere((s) => s is TransactionLoaded)
                as TransactionLoaded;
        expect(
          finalLoaded.transactions.where((t) => t.id == 'srv_1').length,
          equals(1),
        );
